...
class Draggable extends React.Component {
    componentDidMount() {
        ...
        // in this case, the Draggable: https://jqueryui.com/draggable/
        this.$node.draggable({
            // Get the incoming `opacity` prop and use it in the plugin configuration
            opacity: this.props.opacity,
            // Get the incoming onChange func and we invoke it on the Draggable `change` event
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.enable !== this.props.enable) {
            this.$node.draggable(nextProps.enable ? 'enable' : 'disable');
        }
    }

    // jQuery UI draggable expects a <ul> list with <li>s.
    renderItems() {
...
Draggable.defaultProps = { opacity: 1, enable: true };
